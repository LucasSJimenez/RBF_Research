...
def local_interpolation(xvals, p_degree):
    ...
    for i in range(length):
      ...
      r = X-Y
      function_vals = y_func(np.array(knn))
      ...
      function_vals = np.append(function_vals, 1)
      for k in range(1,p_degree+1):
        function_vals = np.append(function_vals, xvals[i]**(k))
      ...
      #lambda_weights = np.matmul(A_matrix_inv, function_vals)
      extended_knn = np.linspace(start = min(knn), stop = max(knn), num = 25)


      big_knn1, big_knn2 = np.meshgrid(knn, extended_knn)

      r_ext = big_knn1 - big_knn2

      poly_matrix_ext = []
      print(i)
      for w in range(len(extended_knn)):
        row = []
        row.append(1)
        for f in range(1,p_degree+1):
            row.append(extended_knn[w]**(f))
        poly_matrix_ext.append(row)
      phs_matrix_ext = np.ones(len(r_ext[1]))

      for k in range(0,len(r_ext)):
        row = PH_Spline(r_ext[k])
      ...
      top_half_A_ext = np.hstack((phs_matrix_ext.getA(),poly_matrix_ext.getA())) #getA makes the matrix an array which allows us to stack it
      bottom_half_A_ext = np.hstack((poly_matrix_t.getA(), zeroes.getA()))
      A_matrix_ext = np.matrix(np.vstack((top_half_A_ext,bottom_half_A_ext)))



      interpolation_points = np.matmul(A_matrix_ext, lambda_weights.getT())
      interpolation_points = interpolation_points[:len(extended_knn)]
      interpolation_points_graph = []
      for h in range(len(interpolation_points)):
        interpolation_points_graph.append(interpolation_points[h][0])
      interpolation_points = np.array(interpolation_points_graph)

      # Shows each local interpolant

      #plt.scatter(extended_knn, interpolation_points_graph, color = 'red', label = '2') # Data Points
      #plt.show()

      # Shows the local interpolants on one graph

      all_extended_knn.append(extended_knn)
      all_interpolation_points.append(interpolation_points)

      
    for extended_knn, interpolation_points in zip(all_extended_knn, all_interpolation_points):
       plt.plot(extended_knn, np.array(interpolation_points).flatten(), alpha=0.5) 

    plt.xlabel('x')
    plt.ylabel('Interpolated Value')
    plt.title('All Local Interpolations')
    plt.grid(True)
    plt.show()
...
# poly_degree_5_vec = []
# xvals_graph = []
# for i in range(1,6): # Iterates over number of sample points
#   u = 50*i
#   step = (stop-start)/u
#   num_pts = int((((stop-start)/step)+1))
...
# print(poly_degree_2_vec)
# print(poly_degree_3_vec)
# print(poly_degree_4_vec)
# print(poly_degree_5_vec)
